...
static void carve(cv::Mat& cameraMatrix, cv::Mat& distCoeffs, Model& model, cv::Mat& image) {
    ...
    std::cout << "LOG - VC: generated carving mask." << std::endl;
    ...
    for (int x = 0; x < model.getX(); x++)
    {
        for (int y = 0; y < model.getY(); y++)
        {
            for (int z = 0; z < model.getZ(); z++)
            {
                ...
                cv::Point pixel_pos = cv::Point((int)std::round(camera_coord[0] / 100) + image.cols/2, (int)std::round(camera_coord[1] / 100) + image.rows / 2);
